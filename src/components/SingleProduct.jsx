...
function SingleProduct() {
  ...
  return (
    <div className="mt-14 md:mt-20 select-none w-full max-w-[1200px] mx-auto px-5 drop-shadow-xl">
      <Link to={"/market/" + catMatherName()}>
        <h1 className="py-3 text-2xl flex gap-3 items-center text-gray-700 font-medium">
      ...
      >
        <div className="flex-1  flex items-center justify-center">
          <img
            className="h-[300px] w-[300px] object-contain m-2 "
            src={"https://ayshadashboard.com/" + productInfo.image}
            alt=""
          />
        </div>
        ...
        <div className="p-6 md:p-4 w-full md:w-fit md:flex-1 h-full flex flex-col gap-2">
          ...
          <p className=" md:pr-5 text-sm text-gray-600">
            {productInfo.description || null}
          </p>
          {q > 1 && (
            <h3 className=" text-xl font-medium text-gray-700">
              {productInfo.price} DH * {q}
            </h3>
          )}
          <h3 className="py-1 text-3xl font-medium text-gray-700">
            {productInfo.price * q} DH
          </h3>
          <div className="hover:scale-105 flex items-center justify-center gap-1 drop-shadow-md bg-white rounded-full w-fit border ">
            <IconButton onClick={() => handelQChange(-1)}>
          ...
          <div className="flex-1 flex gap-2 justify-center sm:justify-end">
            ...
            <button className="flex-1 sm:flex-none hover:scale-105 ease-in-out duration-300 button bg-[#95BF6D] text-white text-xs md:text-md ">
              {" "}
              Buy now{" "}
            </button>
            <button
              className=" sm:flex-none hover:scale-105 ease-in-out duration-300 button bg-gray-600 text-white text-xs md:text-md"
              onClick={() => addtocartHandler()}
            >
              <AddRoundedIcon /> Add to cart
            </button>
          </div>
        </div>
      </motion.div>
